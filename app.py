...
BINANCE_FUT_BASES = ["https://fapi.binance.com"]
...
for sym in assets:
    ...
    for tf in tfs:
        try:
            ...
            rows.append({"Asset": sym, "TF": tf, "Bias": "ERR", "Error": str(e)})
...
if not table.empty:
    ...
    for _, row in table.iterrows():
        ...
        col1.markdown(f"**{row['Asset']} — {row['TF']}**")
        col1.markdown(f"Bias: {row['Bias']}  |  Weight: `{row['Weight']}`  |  Alignment: {row['Alignment']}")
        if show_sparklines and len(row["Spark"]) > 1:
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                y=row["Spark"],
                mode="lines",
                line=dict(color=row["Trend"], width=2),
                hoverinfo="skip"
            ))
            fig.update_layout(
...
st.caption("Data: Binance (fallback to Kraken). Funding from Binance Futures. Not financial advice.")
